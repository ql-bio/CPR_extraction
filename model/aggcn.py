...
class GCNClassifier(nn.Module):
    def __init__(self, opt, emb_matrix=None):
        ...
        self.bert=BertForSequenceClassification.from_pretrained(opt["bert_model_file"], cache_dir=str(PYTORCH_PRETRAINED_BERT_CACHE), num_labels=opt['num_class'])
        self.classifier = nn.Linear(in_dim+768, opt['num_class'])
        self.opt = opt
        self.init_embeddings()
    
    def init_embeddings(self):
        self.classifier.weight.data.uniform_(-1.0, 1.0)
        self.classifier.bias.data.zero_()
    
    def forward(self, inputs,input_ids,input_mask,segment_ids,label_id,l,b=None):
        outputs, pooling_output = self.gcn_model(inputs,l)
        bert_pooling_output=self.bert(input_ids, segment_ids, input_mask, label_id)
        out=torch.cat([outputs,bert_pooling_output],dim=1)
        logits = self.classifier(out)
        loss_fct = CrossEntropyLoss()
        loss = loss_fct(logits.view(-1, self.opt['num_class']), label_id.view(-1))
        if b is None:
            return loss
        else:
            return logits,loss
